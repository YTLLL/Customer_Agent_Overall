...
class CallingDBService:
    ...
    def __init__(self):
        ...
        db_name = os.getenv("MONGODB_DB_NAME", "calling_db")
        reasoning_db_name = os.getenv("MONGO_DB_NAME", "hotel_db")
        # 连接到MongoDB
        self.client = MongoClient(mongo_uri)
        self.db: Database = self.client[db_name]
        self.reasoning_db: Database = self.client[reasoning_db_name]
    ...
    def get_conversation(self, conversation_id: str) -> Optional[Conversation]:
        ...
        conversation = Conversation(
            ...
            created_at=datetime.fromisoformat(data["created_at"]) if isinstance(data["created_at"], str) else data[
                "created_at"],
            updated_at=datetime.fromisoformat(data["updated_at"]) if isinstance(data["updated_at"], str) else data[
                "updated_at"],
    ...
    def get_active_conversation(self, user_id: str) -> Optional[Conversation]:
        """获取用户的最近一个对话"""
        # 查询MongoDB中该用户的最近一个对话，按时间戳降序排序
        data = self.conversations.find_one({
            "user_id": user_id
        }, sort=[("created_at", -1)])  # -1表示降序排序
